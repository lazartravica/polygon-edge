...
func TestSubmitCheckpoint(t *testing.T) {
	...
	result = transition.Create2(types.Address{123}, exitHelper.Bytecode, big.NewInt(0), 1000000000)
	assert.NoError(t, result.Err)
	ehAddress := result.Address

	ehInit, err := exitHelper.Abi.GetMethod("initialize").Encode([]interface{}{ethgo.Address(rcAddress)})
	require.NoError(t, err)

	result = transition.Call2(senderAddress, ehAddress, ehInit, big.NewInt(0), 1000000000)
	require.NoError(t, result.Err)
	require.True(t, result.Succeeded())
	require.False(t, result.Failed())
	...
	result = transition.Call2(senderAddress, ehAddress, ehExit, big.NewInt(0), 1000000000)
	require.NoError(t, result.Err)
	require.True(t, result.Succeeded())
	require.False(t, result.Failed())

	ehExitCheck, err := exitHelper.Abi.GetMethod("processedExits").Encode([]interface{}{
		exits[0].ID,
	})
	require.NoError(t, err)

	result = transition.Call2(senderAddress, ehAddress, ehExitCheck, big.NewInt(0), 1000000000)
	require.NoError(t, result.Err)
	require.True(t, result.Succeeded())
	require.False(t, result.Failed())
	t.Log(result.ReturnValue[31])
	require.Equal(t, int(result.ReturnValue[31]), 1)
	t.Log(result.ReturnValue)

	ehResultExitCheck, err := exitHelper.Abi.GetMethod("resultExits").Encode([]interface{}{
		exits[0].ID,
	})
	require.NoError(t, err)

	result = transition.Call2(senderAddress, ehAddress, ehResultExitCheck, big.NewInt(0), 1000000000)
	require.NoError(t, result.Err)
	require.True(t, result.Succeeded())
	require.False(t, result.Failed())
	t.Log("resultExits", result.ReturnValue)

	idGetter, err := l1Art.Abi.GetMethod("id").Encode([]interface{}{})
	require.NoError(t, err)
	result = transition.Call2(senderAddress, l1Cntract, idGetter, big.NewInt(0), 1000000000)
	require.NoError(t, result.Err)
	require.True(t, result.Succeeded())
	require.False(t, result.Failed())
	t.Log(result.ReturnValue)

	addrGetter, err := l1Art.Abi.GetMethod("addr").Encode([]interface{}{})
	require.NoError(t, err)
	result = transition.Call2(senderAddress, l1Cntract, addrGetter, big.NewInt(0), 1000000000)
	require.NoError(t, result.Err)
	require.True(t, result.Succeeded())
	require.False(t, result.Failed())
	t.Log(result.ReturnValue)

	dataGetter, err := l1Art.Abi.GetMethod("data").Encode([]interface{}{})
	require.NoError(t, err)
	result = transition.Call2(senderAddress, l1Cntract, dataGetter, big.NewInt(0), 1000000000)
	require.NoError(t, result.Err)
	require.True(t, result.Succeeded())
	require.False(t, result.Failed())
	t.Log("data", result.ReturnValue)

	counterGetter, err := l1Art.Abi.GetMethod("counter").Encode([]interface{}{})
	require.NoError(t, err)
	result = transition.Call2(senderAddress, l1Cntract, counterGetter, big.NewInt(0), 1000000000)
	require.NoError(t, result.Err)
	require.True(t, result.Succeeded())
	require.False(t, result.Failed())
	t.Log("counter", result.ReturnValue)
}
